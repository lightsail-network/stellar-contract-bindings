...
from stellar_sdk import SorobanServer, xdr, Address, StrKey
...
def get_contract_wasm_by_hash(wasm_hash: bytes, rpc_url: str) -> bytes:
    with SorobanServer(rpc_url) as server:
        key = xdr.LedgerKey(xdr.LedgerEntryType.CONTRACT_CODE,
...
def get_wasm_hash_by_contract_id(contract_id: str, rpc_url: str) -> bytes:
    with SorobanServer(rpc_url) as server:
        key = xdr.LedgerKey(xdr.LedgerEntryType.CONTRACT_DATA,
...
@click.option(
    '--contract-id',
    required=True,
    help='The contract ID to generate bindings for'
)
@click.option(
    '--language',
    default='python',
