...
public class ResultSmoke {
    ...
    public static void main(String[] args) {
        ...
        check("Ok arm decoded", ok.getR().isOk() && !ok.getR().isErr() && ok.getR().getValue() == 5L);
        boolean okHasNoError = false;
        try {
            ok.getR().getError();
        } catch (IllegalStateException e) {
            okHasNoError = true;
        }
        check("Ok arm has no error to get", okHasNoError);
        ...
        check("Err arm decoded",
            err.getR().isErr() && err.getR().getError() == Client.Code.Bad);
        boolean errHasNoValue = false;
        try {
            err.getR().getValue();
        } catch (IllegalStateException e) {
            errHasNoValue = true;
        }
        check("Err arm has no value to get", errHasNoValue);
        check("Result value semantics",
            Client.Result.<Long, Client.Code>err(Client.Code.Bad).equals(err.getR())
                && !Client.Result.<Long, Client.Code>ok(5L).equals(err.getR()));
